...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (!ConfigManager::instance()->getCityBazaarTerminalEnabled())
			return GENERALERROR;
		...
		int initCost = ConfigManager::instance()->getCityBazaarTerminalInitCost();
		int maintCost = ConfigManager::instance()->getCityBazaarTerminalMaintCost();
		...
		promptText << "Select Bazaar Terminal from the list below. The Bazaar Terminal will be placed at your current location and rotation(ie. the way you're facing). "
				...
				<< "Placing a Bazaar Terminal will cost " << String::valueOf(initCost) << " credits up front, withdrawn from the treasury at the time of placement. "
				<< "A maintenance fee of " << String::valueOf(maintCost) << " credits will be deducted from the treasury on every city update.";
